...
// --- СТАНДАРТНЫЕ МЕТОДЫ (ДЛЯ ФРОНТЕНДА И АДМИНКИ) ---
...
app.post('/api/integrations/telegram/webhook', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (userRes.rows.length === 0) {
            ...
            if (inviteMatch) {
                // Регистрация водителя по инвайту
                ...
                const newUser = await client.query(
                    `INSERT INTO users (telegram_user_id, full_name, role, tenant_id, login, password_hash, is_active)
                     VALUES ($1, $2, 'driver', $3, $4, '123456', true) RETURNING *`, [tgId, fullName, inviteRes.rows[0].tenant_id, login]);
                ...
                await client.query(`INSERT INTO dict_trucks (tenant_id, name, plate, is_active) VALUES ($1, 'Тестовый Грузовик', 'A777AA 77', true)`, [tenantRes.rows[0].id]);
                await client.query(`INSERT INTO dict_sites (tenant_id, name, address, is_active) VALUES ($1, 'Главный Склад', 'г. Москва', true)`, [tenantRes.rows[0].id]);
                await client.query('COMMIT');
        ...
        if (truckMatch) {
            ...
            return res.json({ action: 'select_site', text: 'Укажите объект:', user });
        }
        ...
        if (cmd === '/end_shift' || cmd === '/end_shift_now' || (cmdText && !cmdText.startsWith('/'))) {
            ...
            if (!shift) {
                ...
                return res.json({ action: pending.rows.length > 0 ? 'status' : 'show_driver_menu', text: pending.rows.length > 0 ? 'Ожидается накладная.' : 'Активной смены нет.', user });
            }
            if (!cmdText.startsWith('/')) await client.query(`UPDATE shifts SET comment = $1 WHERE id = $2`, [cmdText, shift.id]);
            if (shift.odometer_required && !shift.photo_end_url) return res.json({ action: 'ask_photo', text: '📸 Пришлите фото одометра для закрытия.', user });
            if (user.tenant_invoice_required) {
                await client.query(`UPDATE shifts SET status = 'pending_invoice' WHERE id = $1`, [shift.id]);
        ...
        return res.json({ action, text: 'Главное меню', user });
